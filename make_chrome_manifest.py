...
so nothing else in this manifest or in background.js/popup.js/options.js
needs to change for Chrome -- just a floor on which Chrome this build
actually runs on.
...
# Promise, which options.js's saveOptions() awaiting sendMessage() depends on.
